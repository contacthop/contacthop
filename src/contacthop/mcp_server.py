...
def build_server(
    ...
    def one(model: Any) -> dict:
        result: dict = model.model_dump(mode="json")
        return result

    def many(models: list[Any]) -> list[dict]:
    ...
    async def list_contacts() -> list[dict]:
        ...
        data: list[dict] = await hop._request("GET", "/v1/contacts?limit=200")
        return data
    ...
    @server.tool()
    async def contact_stats(contact_id: str) -> dict:
    ...
    async def list_conversations(status: str | None = None) -> list[dict]:
        ...
        data: list[dict] = await hop._request("GET", path)
        return data
    ...
    @server.tool()
    async def place_call(conversation_id: str, opening_line: str | None = None) -> dict:
